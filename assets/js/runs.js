...
const statusTag = r => {
  if(r.state === 'live')      return `<span class="stag live-tag">● LIVE</span>`;
  if(r.state === 'analysing') return `<span class="stag ana-tag">◴ analysing</span>`;
...
function winTag(r, hw){
  ...
  return `<span class="tag" style="background:${hw.color};color:var(--paper);border-color:var(--ink)" title="${hw.fullName}">${hw.shortName}${org}</span>`;
}

function renderList(list){
  document.getElementById('runs').innerHTML = list.map(r => {
    const h1 = H.get(r.teams.team1.model), h2 = H.get(r.teams.team2.model);
    const hw = (r.winner && r.winner !== 'draw') ? H.get(r.teams[r.winner].model) : null;
    const inner = `
      <div class="rdate">${dateLabel(r.date, r.time)}</div>
      <div>
        <div class="rname">${r.name}${r.category ? ` <span class="badge ${r.category.toLowerCase()}">${r.category}</span>` : ''} ${statusTag(r)}</div>
      ...
      <div class="rwin">${r.winner ? winTag(r, hw) : ''}<span class="rounds">${r.state === 'live' ? 'watch live' : 'view thread'}</span></div>
      <div class="rgo">→</div>`;
    return `<a class="run live" href="trajectory.html?run=${r.id}">${inner}</a>`;
  }).join('');
}
renderList(runs);
